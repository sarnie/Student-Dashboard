    ...
    while ($row = $stmt->fetch()) {
        ...
        if ($row->blogText != '') {
            ...
            if ($row->comment == '') {
                ...
                $theHtml .= '<div class="hide media-container"><div class="media clearfix"> <a class="pull-left" href="#"> <img src="img/profile-pic.jpg" class="media-object" alt="64x64" /> </a>';
                $theHtml .= ' <div class="media-body"><h4 class="media-heading">Oluwatosin</h4>' . $row->comment . '</div>';
            }
            $theHtml .= ' </div></div></div>';
...
function addThePost($uId, $postContent) {
    $dbHandle = new DBConnection();
    $sql = "INSERT INTO blog (`blogText`, `datePosted`, `studentID`) VALUES ('" . $postContent . "',NOW()," . $uId . ")";
    $stmt = $dbHandle->prepare($sql);
    $stmt->execute();
    echo 'Done';
}
...
function getMeeting_st($uId, $ajx = 1) {
    ...
    $theHtml = '<table class="table table-striped">
              <thead>
                <tr>
                  ...
                  <th>Details (staff)</th>
                  <th>Meeting Date</th>
                  <th>Meeting Time (hh:mm)</th> </tr> </thead>';
    ...
    while ($row = $stmt->fetch()) {
        ...
        $theHtml .= '<td>N/A</td> <td>' . date_format($mDate, 'd/m/Y') . '</td> <td>' . date_format($mTime, 'G:i') . '</td>';
        $theHtml .= '</tr>';
        $x++;
    }
...
function getUploadedCW_st($studentID, $ajx = 1) {
    ...
    while ($row = $stmt->fetch()) {
        ...
        if ($row->comment != '') {
            $theHtml .= '<tr><td colspan="4">' . $row->comment . '</td></tr>';
...
function getMeetingSummary($studentID) {
    ...
    $sqlCanceled = "SELECT * FROM meeting WHERE studentID = " . $studentID . " AND happened = 2";
    ...
    $theHtml .= '<tr><td>Real</td><td>' . $numVirtual . '</td><td align="center"><a href="meetings.php?rq=1" class="btn btn-success">Request</a></td></tr>';
    $theHtml .= '<tr><td>Virtual</td><td>' . $numReal . '</td><td align="center"><a href="meetings.php?rq=1" class="btn btn-success">Request</a></td></tr>';
    $theHtml .= '<tr><td>Canceled</td><td colspan="2">' . $numCanceled . '</td></tr>';
    echo $theHtml;
}
...
function getLastMeetingDate($studentID) {
    $dbHandle = new DBConnection();
    $sql = "SELECT * FROM meeting WHERE studentID = " . $studentID . " ORDER BY meetingDate DESC";
    $stmt = $dbHandle->prepare($sql);
    $stmt->execute();
    $stmt->setFetchMode(PDO::FETCH_OBJ);
    $x = 1;
    while ($row = $stmt->fetch()) {
        ...
        $theHtml = date_format($meetingDate, 'd/m/Y');
...
function getThePost_tu($uID) {
    $dbHandle = new DBConnection();
    if(!is_superAdmin()){
        $sql = "SELECT blog.* FROM staff INNER JOIN (student INNER JOIN blog ON student.studentID = blog.studentID) ON staff.staffID = student.staffID WHERE (((student.staffID)=" . $uID . "))";
    }else{
...
function getMeeting_tu($uId, $ajx = 1, $status = 'Pending') {
    ...
    while ($row = $stmt->fetch()) {
        ...
        if ($status == 'Pending' && !is_superAdmin()) {
            $theHtml .= '<td> <form role="form" id="meeting-action"> 
          <input type="hidden" id="meetAct" /><div class="btn-group">
  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
    Action <span class="caret"></span>
  </button>
  <ul class="dropdown-menu pull-right" role="menu">
    <li><a href="" class="acc" value="Accept"><span class="glyphicon glyphicon-ok accept"></span> Accept<span class="hide">'.$row->meetingID.'</span></a></li>
    <li class="divider"></li>
     <li><a href="" class="dec" value="Decline"><span class="glyphicon glyphicon-remove decline"></span> Decline<span class="hide">'.$row->meetingID.'</span></a></li>
  </ul>
</div> </form></td>';
...
function getUploadedCW_tu($uID, $ajx = 1) {
    ...
    while ($row = $stmt->fetch()) {
        ...
        if ($row->comment == '' && !is_superAdmin()) {
            $theHtml .= '<tr><td colspan="4" id="'.$row->uploadID.'">
                <h4 class="upCommentTitle">Comment</h4>
<p class="hide"></p>                
<a href="" class="btn btn-success btn-sm addCommentBtn">Add Comment</a>
			<div class="addComment hide"><form role="form" id="add-upload-comment"><input type="hidden" value="7" class="blogId"><div class="form-group"><textarea class="form-control" rows="3" placeholder="Enter your comment.." id="p-comment"></textarea> </div> <div class="form-group"><button type="submit" class="btn btn-success btn-sm">Add Comment</button></div></form></div>
			</td></tr>';
            $theHtml .= '<tr><td colspan="4">&nbsp;</td></tr>';
        } else {
...
function filterByTutorState($theValue) {
    ...
    while ($row = $stmt->fetch()) {
        ...
        $theHtml .= '<td>' . $row->firstName . '</td> <td>' . $row->lastName . '</td>
				  ...
				  <td>' . getStaffFullName($row->staffID) . '</td>
                  <td class="text-center">' . date_format($theDate, 'd/m/Y') . '</td>
                  <td class="text-center">' . studentSentMsgCount($row->email) . '</td>
                </tr>';
    }
...
$dbHandle = new DBConnection();
    ...
    $stmt->execute();
    $stmt->setFetchMode(PDO::FETCH_OBJ);
    $currentUrl =  'http://'.$_SERVER['HTTP_HOST'].'/su/index.php';
   
    while ($row = $stmt->fetch()) {
        $theHtml .= '<option value="'.$currentUrl.'?id='.$row->studentID.'"> '.$row->firstName.' '.$row->lastName.' </option>';
    }
